...
Nền tảng định danh số gồm hai trụ cột: **eKYC** (xác minh người thật) và **IdP/SSO** (OAuth 2.0 / OpenID Connect), theo mô hình **một pool người dùng** và **tự chủ** (tự viết IdP, không Keycloak). Các ứng dụng tích hợp dưới dạng **relying party (client)**. Tái sử dụng house architecture O2O ở mức convention. Đây là **nền tảng cấp nhà nước, phục vụ Bộ Công an** (định danh công dân quốc gia, gắn VNeID/CSDL dân cư theo **Đề án 06**) → chịu ràng buộc **chủ quyền & an ninh quốc gia** (ADR-0014).
...
- **Dự án nhà nước & công an (ADR-0014):** chủ quyền & lưu trú dữ liệu trong nước; **hạ tầng trong nước/on-prem** (KHÔNG cloud nước ngoài); **VNeID/CSDL quốc gia về dân cư (C06)** bắt buộc theo **Đề án 06**; PKI/chữ ký số theo **CA quốc gia (NEAC)/Ban Cơ yếu**; an toàn HTTT theo cấp độ (NĐ85/2016, TCVN 11930).
- Tuân thủ **Nghị định 13/2023/NĐ-CP**, **Luật An ninh mạng 2018 + NĐ53/2022**, **Luật Căn cước 2023**.
...
- *Hệ ngoài:* **VNeID** (nguồn xác minh chính thống, MVP), nhà cung cấp eKYC (FPT.AI…), object storage/CDN, KMS/Vault (master key).
...
**Level 2 — `identity-core`:** module `auth` (OIDC tự viết), `user`, `ekyc` (adapter **VNeID** + FPT.AI), `IAL` (gắn năng lực), `admin` (quản lý client/RP). Layering DDD. Chi tiết: Spec #1.
...
- **PostgreSQL** primary + read replica; shard theo thời gian/khối khi cần. **Redis Sentinel**. **Kafka**. **Object storage + CDN trong nước/on-prem** (cloud nội địa Viettel/VNPT/FPT/CMC — KHÔNG Cloudflare/R2/S3 nước ngoài; ADR-0014).
...
- **Chủ quyền (ADR-0014):** toàn bộ hạ tầng & dữ liệu đặt **trong lãnh thổ VN**; MASTER_KEY/HSM trong nước (cân nhắc Ban Cơ yếu); cấp độ ATTT cao → SOC + kiểm định độc lập.
...
> **Nguồn chân lý:** ADR đầy đủ nằm ở `docs/adr/` (0001–0014). Dưới đây chỉ là tóm tắt.
...
- **ADR-0014 (chi phối tất cả) — Dự án nhà nước & công an.** Chủ quyền dữ liệu, hạ tầng trong nước/on-prem, VNeID/CSDL dân cư bắt buộc, PKI quốc gia, cấp độ ATTT — buộc xem lại các ADR hạ tầng/khóa/PKI.
